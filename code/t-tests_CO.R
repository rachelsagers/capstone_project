...
#splitting data into two data frames for pre and post to compare 
MIS_pre_CO <- MIS_CO_preandpost %>% 
  filter(pre_or_post == "pre") 

MIS_post_CO <- MIS_CO_preandpost %>% 
  filter(pre_or_post == "post") 

#t-test between daily mean of pre and post time periods 
t.test(MIS_pre_CO$daily_mean, MIS_post_CO$daily_mean) 


### Chapel Hill ### 

#splitting data into two data frames for pre and post to compare 
CHP_pre_CO <- CHP_CO_preandpost %>% 
  filter(pre_or_post == "pre") 

CHP_post_CO <- CHP_CO_preandpost %>% 
  filter(pre_or_post == "post") 

#t-test between daily mean of pre and post time periods 
t.test(CHP_pre_CO$daily_mean, CHP_post_CO$daily_mean) 


### Corvallis ### 

#splitting data into two data frames for pre and post to compare 
COR_pre_CO <- COR_CO_preandpost %>% 
  filter(pre_or_post == "pre") 

COR_post_CO <- COR_CO_preandpost %>% 
  filter(pre_or_post == "post") 

#t-test between daily mean of pre and post time periods 
t.test(COR_pre_CO$daily_mean, COR_post_CO$daily_mean) 


### Olympia ### 

#splitting data into two data frames for pre and post to compare 
OLY_pre_CO <- OLY_CO_preandpost %>% 
  filter(pre_or_post == "pre") 

OLY_post_CO <- OLY_CO_preandpost %>% 
  filter(pre_or_post == "post") 

#t-test between daily mean of pre and post time periods 
t.test(OLY_pre_CO$daily_mean, OLY_post_CO$daily_mean) 
